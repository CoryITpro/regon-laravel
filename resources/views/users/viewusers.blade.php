                                        ...
                                        <table id="usertable" class="table table-striped table-bordered" style="width:100%">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{ $user->address }}</td>
                                                    @if ($user->premium == 0)
                                                    <td><a class="btn btn-secondary" href="{{ route('premiumuser',$user->id) }}">No</a></td>
                                                    @else
                                                    <td><a class="btn btn-primary" href="{{ route('premiumuser',$user->id) }}">Yes</a></td>
                                                    @endif
                                                    <td>
                                                        @if ($user->premium)
                                                        {{ (new Carbon\Carbon($user->premium))->addDays(183)->format('Y-m-d') }}
                                                        @endif
                                                    </td>
                                                    <td>{{ $user->last_login }}</td>
                                                    <td><button type="button" class="btn btn-primary" data-name="{{ $user->name }}" data-toggle="modal" data-target="#edituser" data-id="{{ $user->id }}" data-email="{{ $user->email }}" data-verified="{{ $user->verified }}" data-premium="{{ $user->premium }}" data-name="{{ $user->name }}">
...
<script>
    $('#edituser').on('show.bs.modal', function(event) {
        ...
        if (button.data('verified'))
            modal.find('.modal-body #verified').attr('checked', true)
        else
            modal.find('.modal-body #verified').attr('checked', false)
